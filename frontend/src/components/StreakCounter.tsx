...
const StreakCounter: React.FC<CalendarProps> = ({ submissionCalendar }) => {
  const currentStreak = React.useMemo(() => {
    ...
    while (true) {
      ...
      // Check for current day submissions or "ice" (missed but part of a streak)
      if (dailyCounts[currentDayTs] && dailyCounts[currentDayTs] > 0) {
        streak += 1;
        missedStreakDay = false; // Reset missed streak day
      } else if (!missedStreakDay) {
        // Allow one missed day (count as "ice")
        streak += 1;
        missedStreakDay = true;
      } else {
        // Break if a second consecutive miss occurs
        break;
      }
  ...
  return (
    <div className={`streak-counter ${currentStreak > 0 ? 'active' : 'inactive'}`}>
      <div className="streak-content">
        <svg
          className="fire-icon"
          xmlns="http://www.w3.org/2000/svg"
          fill={currentStreak > 0 ? '#ff4500' : '#ccc'}
          height="24px"
          width="24px"
          viewBox="0 0 800 800"
        >
        ...
        <p><b>{currentStreak} day{currentStreak !== 1 ? 's' : ''}</b></p>
      </div>
    </div>
  );
};
